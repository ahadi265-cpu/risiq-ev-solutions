...
# Cinematic motion architecture (2026-09-22)
New deps: `lenis`, `three`, `@react-three/fiber` (+ `@types/three`). `three` is a manual chunk and only loads via the lazy `HeroField`.
- **Smooth scroll — `lib/useLenis.ts`.** One Lenis instance on `window`, `autoRaf: false`, driven from `gsap.ticker`, `lenis.on('scroll', ScrollTrigger.update)`. **Not created** on `(pointer: coarse)` or reduced motion — native scrolling stays. `scrollToTop()`/`scrollToEl()` route through it; the route-change reset and the hero's `#audience` link use them. `DialogContent` carries `data-lenis-prevent` so wheel inside a modal scrolls the modal. `lenis/dist/lenis.css` is imported by the hook.
- **WebGL hero — `components/HeroField.tsx`** (default export, lazy). R3F `Canvas` (dpr ≤ 1.5, low-power, alpha) with a 2 400-point field and 26 wireframe "battery cells" (`InstancedMesh`), both leaning toward the cursor via a module-level `mouse` ref. Palette follows the `.dark` class through a `MutationObserver`: white over the red band, cyan/emerald additive over obsidian. Mounted only when `!reduce`; the CSS aurora/grid/scan layers remain underneath as the always-on fallback.
- **Pinned horizontal sequence — `components/WorkflowScroller.tsx`** replaces the four step cards on Home. `gsap.matchMedia('(min-width: 1024px) and (prefers-reduced-motion: no-preference)')` pins the `h-dvh` section and scrubs the track's `x` by `scrollWidth - clientWidth`; below `lg` / under reduced motion it is a plain vertical stack (no JS repositioning, copy always reachable). Progress bar + "0N / 04" from `onUpdate`. Layout calls `ScrollTrigger.refresh()` 120 ms after each route change so the pin re-measures.
- **Scroll-scrubbed signal — `SocketSimulator`:** in socket mode a `.scrub-draw` overlay of `BUS_FULL` draws itself (dashoffset L→0) and a `.scrub-carrier` rides `#bus-full` via `MotionPathPlugin`, both scrubbed `top 88% → bottom 30%`. The overlay sits before the meter group in DOM so the meter covers the middle. SMIL carriers still run autonomously on top.
- **Texture — `.noise-overlay`** (fixed, z-45: above chrome, below dialogs), inline feTurbulence SVG, `mix-blend-mode: overlay`, opacity 0 in light / 0.22 in `.dark`, hidden in print.
- **Headless verification note:** WebGL needs `--use-angle=swiftshader --enable-unsafe-swiftshader` (not `--disable-gpu`); Lenis only reacts to real wheel events (`Input.dispatchMouseEvent mouseWheel`), `window.scrollTo` bypasses it.
- **Two defects found while verifying, both fixed:** (1) the Google Fonts `<link>` in `index.html` was render-blocking — on a slow font fetch the page stayed blank for 20 s+; it now loads as `media="print" onload="this.media='all'"` with a `<noscript>` fallback (h1 paints in ~370 ms). (2) The route-transition wrapper (`.animate-rise`, fill-mode `both`) kept an identity `transform` forever, which made the pinned section's `position: fixed` resolve against the page instead of the viewport (the section vanished off the top while pinned). `Layout`'s `RouteEnter` now drops the class on `animationend` and calls `ScrollTrigger.refresh()`. **Rule:** nothing above `<main>`'s content may keep `transform`, `filter`, `will-change` or `contain` once settled, or every pinned section breaks.

# Optical/motion refinement pass (2026-09-26)
Third-party addition: `@react-three/postprocessing` (+ its `postprocessing`/`maath`/`n8ao` deps).
- **`HeroCanvas.tsx`** (renamed from `HeroField.tsx`): particles/cells now repel from a raycaster-projected world cursor position with a real spring (velocity + damping, not the old group-rotation tilt). Postprocessing (`SelectiveBloom` targeted at the particle/cell refs via `selection`, `ChromaticAberration` at a 0.0008 offset, faint `Noise`) is split into its **own lazy chunk, `HeroPostFX.tsx`**, mounted only when `dark && !small` — light-mode visitors (the default theme) never download it. `vite.config.ts`'s `manualChunks` had to be taught to exclude `@react-three/postprocessing`/`postprocessing`/`maath`/`n8ao` from the forced `'three'` vendor bucket, or Rollup silently re-merged the lazy chunk back into the main three bundle every visitor pays for.
- **`MagneticButton.tsx`**: built on `motion.create(Slot)` (Radix Slot, not a wrapping div) so it composes with `DialogTrigger asChild` without breaking accessibility or click wiring. **Real bug caught in verification and fixed:** the first version didn't forward the `onClick`/`aria-*`/`ref` that Radix's own Slot clones onto it — Radix's injected props land on `MagneticButton`'s *props*, not automatically on its rendered output, so the dialog silently stopped opening. Needs `forwardRef` + an explicit `...rest` spread down to the inner `MotionSlot`. Typed the rest bucket as `ComponentPropsWithoutRef<'button'>` omitting `onDrag*/onAnimationStart/End` — Motion redefines those with incompatible signatures. Wired into all four hero/CTA-banner triggers on Home.
- **`WorkflowScroller.tsx`**: background grid now parallaxes at 0.5× via `gsap.set` inside the *same* `onUpdate` that drives the pin (one progress value driving both, not two independently-scrubbed ScrollTriggers) — the ratio is exact by construction. Each card's icon/number chip gets a bounded ±12% `xPercent` drift via `containerAnimation` (GSAP's documented pattern for per-panel motion inside a pinned horizontal scroller), `power3.out`; the active step's chip gets an `expo.inOut` scale pulse on change. The scrub tween itself stays `ease:'none'` — GSAP's own guidance, since eased scrub decouples the track from the scrollbar.
- **`CellHeatmap.tsx`**: the 96 individual per-cell tooltips are now one floating tooltip that spring-snaps (`useSpring`, stiffness 520/damping 30) to whichever cell is hovered/focused/clicked — fewer DOM nodes at rest, and it visibly "attaches" rather than appearing in place.
- **Fluid typography**: `--fs-hero/h1/h2/h3` clamp() tokens + opt-in `.text-fluid-*` utility classes in `index.css`. Applied to the Home hero h1, all six other page h1s, and `SectionHead`'s h2 (used by every section headline site-wide). **Deliberately not** a bare `h1{}`/`h3{}` selector: h3 in particular is reused for everything from a real content header to a tiny mono eyebrow label, and a blanket rule would wreck the latter — the opt-in-only-where-applied approach sidesteps that by construction, so `.text-fluid-h3` exists but is applied narrowly.
- `.noise-overlay`: `mix-blend-mode: soft-light` (from `overlay`), dark opacity 0.15 (from 0.22).
- **Verification lesson worth keeping:** a background/track parallax ratio read as exactly 1.0 (broken) in testing; root cause was the *test probe's* `querySelector('.will-change-transform')` matching whichever element had that class first in DOM order — both the background layer and the track carried it, so both "readings" were actually the same element. Confirmed with a more specific `:not()` selector before concluding anything was broken in the app itself; a first "fix" that was never actually necessary was applied, then reasoned back out with a corrected code comment. Same underlying lesson as the HeroPostFX network-log false negative: a failing verification script is not automatically the app being wrong — chase evidence at more than one layer before rewriting product code.
