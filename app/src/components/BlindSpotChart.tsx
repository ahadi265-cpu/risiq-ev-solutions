  ...
  return (
    <ResponsiveContainer>
      <LineChart data={data} margin={{ top: 8, right: 12, bottom: 0, left: -16 }}>
        ...
        <XAxis dataKey="km" type="number" domain={[0, 100000]} tickCount={6} tickLine={false} axisLine={{ stroke: 'var(--border)' }}
          tick={tick} tickFormatter={(v: number) => `${v / 1000}k`} />
        <YAxis domain={[80, 100]} tickLine={false} axisLine={false} width={44} tick={tick} tickFormatter={(v: number) => `${v}%`} />
        <Tooltip contentStyle={{ borderRadius: 12, border: '1px solid var(--border)', background: 'var(--card)', color: 'var(--card-foreground)', fontSize: 12 }}
          ...
          formatter={(v, n) => [`${Number(v).toFixed(1)}%`, n === 'guess' ? 'Mileage-only guess' : n === 'gentle' ? 'Measured · gentle AC' : 'Measured · fast-DC taxi']} />
        <Legend verticalAlign="top" height={28} iconType="plainline"
          formatter={(v) => <span style={{ color: 'var(--muted-foreground)', fontSize: 12 }}>{v === 'guess' ? 'What the odometer implies' : v === 'gentle' ? 'Socket-measured · gentle AC' : 'Socket-measured · fast-DC taxi'}</span>} />
